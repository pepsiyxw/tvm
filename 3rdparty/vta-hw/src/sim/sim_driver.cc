...
class BitPacker {
 public:
  explicit BitPacker(void* data) {
    data_ = static_cast<uint64_t*>(data);
  }

  uint64_t GetUnsigned(uint32_t index) const {
    if (bits == 64){
      return data_[index];
    } else if (bits == 32) {
  ...
  int64_t GetSigned(uint32_t index) const {
    if (bits == 64) {
      ...
      int64_t uvalue = static_cast<int64_t>(
  ...
  void SetUnsigned(uint64_t index, uint64_t value) {
    if (bits == 64) {
      data_[index] = value;
    }else if (bits == 32) {
  ...
  void SetSigned(uint64_t index, int64_t value) {
    if (bits == 64) {
      ...
      uint32_t shift = (index % kNumPackElem) * bits;
      data_[offset] &= (~(kMask << shift));
      data_[offset] |= static_cast<uint64_t>(value & kMask) << shift;
    }
  }

 private:
  uint64_t* data_;
  static constexpr uint32_t kNumPackElem = 64 / bits;
  static constexpr uint64_t kMask = ((static_cast<uint64_t>(1) << (bits >= 64U ? 63U : bits)) - 1U);
};
...
class Device {
 ...
 private:
  ...
  void RunStore(const VTAMemInsn* op) {
    printf("RunStore execute\n");
    if (op->x_size == 0) return;
    if (op->memory_type == VTA_MEM_ID_OUT) {
      printf("VTA_MEM_ID_OUT == %d\n",VTA_MEM_ID_OUT);
      printf("VTA_OUT_WIDTH == %d\n",VTA_OUT_WIDTH);
  ...
  void RunGEMM(const VTAGemInsn* op) {
    ...
    if (!op->reset_reg) {
          ...
          for (uint32_t uindex = op->uop_bgn; uindex < op->uop_end; ++uindex) {
            ...
            for (uint32_t i = 0; i < VTA_BATCH; ++i) {
              for (uint32_t j = 0; j < VTA_BLOCK_OUT; ++j) {
                ...
                int64_t sum = acc.GetSigned(acc_offset);
                for (uint32_t k = 0; k < VTA_BLOCK_IN; ++k) {
                  sum +=
                      inp.GetSigned(i * VTA_BLOCK_IN + k) *
                      wgt.GetSigned(j * VTA_BLOCK_IN + k);
                  // printf("sum(%d) = inp(%d) x wgt(%d) = %d x %d = %d\n",
                  //     acc_offset, i * VTA_BLOCK_IN + k, j * VTA_BLOCK_IN + k,
                  //     inp.GetSigned(i * VTA_BLOCK_IN + k),
                  //     wgt.GetSigned(j * VTA_BLOCK_IN + k),
                  //     sum);
                } 
      ...
      printf("gemm reset passed\n");
    }
  }

  void RunALU(const VTAAluInsn* op) {
  printf("RunALU execute\n");
    if (op->use_imm) {
      RunALU_<true>(op);
    } else {
      RunALU_<false>(op);
  ...
  void RunALU_(const VTAAluInsn* op) {
    switch (op->alu_opcode) {
      case VTA_ALU_OPCODE_ADD: {
        return RunALULoop<use_imm>(op, [](int64_t x, int64_t y) {
            return x + y;
          });
      }
